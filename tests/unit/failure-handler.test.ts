...
describe("exponentialBackoff", () => {
  it("applies delay of initialDelay * 2^(failures-1)", () => {
    ...
    const before = Date.now();
    const result = handler({ executionTime, consecutiveFailures: 1, taskData: null, error: new Error() });
    expect(result).not.toBeNull();
  ...
  it("doubles delay on each consecutive failure", () => {
    ...
    const before = Date.now();
    const result2 = handler({ executionTime, consecutiveFailures: 2, taskData: null, error: new Error() });
    const result3 = handler({ executionTime, consecutiveFailures: 3, taskData: null, error: new Error() });
  ...
  it("uses now as base when executionTime is in the past", () => {
    ...
    const before = Date.now();
    const result = handler({ executionTime: pastTime, consecutiveFailures: 1, taskData: null, error: new Error() });
    expect(result!.getTime()).toBeGreaterThanOrEqual(before + 1000);
  });
});
...
describe("fixedDelay", () => {
  ...
  it("uses executionTime as base when it is in the future", () => {
    ...
    const result = handler({ executionTime: futureTime, consecutiveFailures: 1, taskData: null, error: new Error() });
